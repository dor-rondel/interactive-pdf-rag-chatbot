...
import { ChatRequest } from '@/app/lib/llamaindex/types';

/**
 * POST handler for chat API endpoint.
 * Processes user messages through RAG pipeline and returns AI responses with sources.
 * Always returns streaming responses with memory for conversational experience.
 *
 * @param request - Next.js request object containing chat message
 * @returns Promise resolving to streaming response with message chunks and sources
 */
export async function POST(request: NextRequest) {
  try {
    ...
    // Always use streaming RAG query with memory
    const { stream, sources } = await queryRAG(body.message);
